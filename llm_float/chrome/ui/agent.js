...
const AGENT_TOOL_DEFS = [
  ...
  { type: "function", function: { name: "fs_read", description: "【读本地文件】读取已授权工作目录内的文本文件。注意：只传相对路径（相对于授权目录），不要传绝对路径。优先用 startLine/endLine 按行读省 token；不传则按字符 offset/limit", parameters: { type: "object", properties: { path: { type: "string", description: "相对路径，如 src/main.py" }, startLine: { type: "number", description: "起始行号（1-based），与 endLine 配合按行读" }, endLine: { type: "number", description: "结束行号（1-based），不传则读到文件末尾" }, offset: { type: "number", description: "起始字符偏移（不传 startLine 时才用）" }, limit: { type: "number", description: "本次最多读取字符数（不传 startLine 时才用）" } }, required: ["path"] } } },
  { type: "function", function: { name: "fs_write", description: "【写本地文件】写入已授权工作目录内的文件（不存在则创建，注意是覆盖写入）。注意：只传相对路径（相对于授权目录），不要传绝对路径", parameters: { type: "object", properties: { path: { type: "string", description: "相对路径" }, content: { type: "string", description: "文件内容" } }, required: ["path", "content"] } } },
  { type: "function", function: { name: "fs_find", description: "【找本地文件】在已授权工作目录内递归查找文件名匹配的文件（glob，如 *.py、data/*.json）。返回相对路径。注意：只传相对路径模式", parameters: { type: "object", properties: { pattern: { type: "string", description: "文件名 glob 模式" } }, required: ["pattern"] } } },
  ...
  { type: "function", function: { name: "web_fetch", description: "【抓网页】后台抓任意 URL 返回纯文本。默认直接 fetch（快，不执行 JS）；如果是 Vue/React SPA 页面（内容空），传 render=true 用后台标签页加载并执行 JS 后再抓。优先于 navigate（不影响当前页面）", parameters: { type: "object", properties: { url: { type: "string", description: "完整 URL" }, render: { type: "boolean", description: "是否渲染 JS（SPA 页面传 true，默认 false）" }, maxChars: { type: "number", description: "最多返回字符数（默认8000，最大20000）" } }, required: ["url"] } } },
  { type: "function", function: { name: "web_post", description: "【POST请求】后台发 POST 请求（跨域不受限）。body 是 JSON 对象字符串（自动设 Content-Type: application/json）；如果传 key=value&key2=value2 格式则自动按表单提交。headers 传 JSON 字符串加自定义请求头如 Authorization。返回状态码、响应头和响应体", parameters: { type: "object", properties: { url: { type: "string", description: "请求 URL" }, body: { type: "string", description: "POST 请求体：JSON 字符串或表单编码" }, headers: { type: "string", description: "额外请求头 JSON（可选），如 {\"Authorization\": \"Bearer xxx\"}" } }, required: ["url", "body"] } } },
...
const AGENT_TOOLS = {
  ...
  web_fetch: async (p) => bridgeCmd("web_fetch", { url: (p && p.url) || "", render: !!(p && p.render), maxChars: (p && p.maxChars) || 0 }),
  web_post: async (p) => bridgeCmd("web_post", { url: (p && p.url) || "", body: (p && p.body) || "", headers: (p && p.headers) || "" }),
